...
def get_xy(folder, label):
    ...
    for filename in os.listdir(folder):
        if filename.endswith('.csv'):
            ...
            data = df['data']
            data_features = FeatureExtract.extract_features(data)
            data_list.append(data_features)
            labels.append(label)
...
def cnn(data, labels, data2, labels2, rs, rs2, epochs = 300, batch_size = 24):    #, rs
    ...
    model = Sequential([
                        # Conv1D(filters = 3, kernel_size = 6, activation = 'relu', input_shape = (N, 1)),
